    ...
    @if($images != null)
        @foreach($images as $image)
            <div class="row" style="margin-left: 0 ">
                @php
                    $image_title = "";
                    if(isset(json_decode($dataTypeContent->image_titles,true)[$loop->iteration-1])){
                    $image_title = json_decode($dataTypeContent->image_titles,true)[$loop->iteration-1];
                    }
                    $image_alt = "";
                ...
                @if($edit)
                    <div class="col-4">
                        <div class="form-group" style="display: flex; flex-direction:column">
                            <label class="control-label" style="font-weight: 900" for="title{{ $loop->iteration }}">Title</label>
                            <input class="form-control" name="image_titles[{{ $loop->iteration-1 }}]" id="title{{ $loop->iteration }}" value="{{ $image_title }}">
                        </div>
                        {{-- <div class="form-group" style="display: flex; flex-direction:column">
